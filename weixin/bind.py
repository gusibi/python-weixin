...
def bind_method(**config):
    class WeixinAPIMethod(object):
        ...
        def _build_parameters(self, args, kwargs):
            for index, value in enumerate(args):
                if value is None:
                    continue
                try:
                    self.parameters[self.accepts_parameters[index]] = encode_string(
                        value
                    )
                except IndexError:
        ...
        def _do_api_request(
            self, url, method="GET", body=None, json_body=None, headers=None
        ):
            headers = headers or {}
            ...
            try:
                ...
                raise WeixinClientError(
                    "Unable to parse response, not valid JSON.", status_code=status_code
                )
        ...
        def execute(self):
            ...
            if self.as_generator:
                ...
                content, next = self._do_api_request(
